...
class ilApiTeams implements ilApiInterface
{
    ...
    public static function getAccessTokenDirect(string $clientId, string $clientSecret, string $tenantId)
    {
        // TeamsClientCredentials
        $scope = 'https://graph.microsoft.com/.default';
        $guzzle = new \GuzzleHttp\Client();
                    ...
                    if (isset($ret->getBody()["value"])) {
                        ...
                        for ((int) $k = 0; $k < count($ret->getBody()["value"]); $k++) {
                            ...
                            if ($LPMode == ilObjMultiVc::LP_ACTIVE) {
                                ...
                                $percentage = 0;
                                if ($sumTimeDiff > 0) {
                                    $percentage = $sumSeconds * 100 / $sumTimeDiff;
                                }
                                if ($percentage > 100) {
                                    $percentage = 100;
                                }
                                ...
                                if ($userId > 0) {
                                    ilLPStatus::writeStatus($objId, $userId, $status, $percentage, true);
                                }
                            }
                        }
                    }
