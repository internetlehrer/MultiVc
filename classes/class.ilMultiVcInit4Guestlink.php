...
require_once($absDirPrefix . "/Services/Utilities/classes/class.ilUtil.php");
 ...
 *
 */
class ilMultiVcInit4Guestlink extends ilInitialisation
{
    ...
    public static function defineClientId( $client_id ) {
...
class JoinMeetingByGuestLink
{
    ...
    private function setHtmlDocument()
    {
        ...
        if( $this->isPwEnabled() && !$this->isUserLoggedIn() ) {
            if( isset($_POST['guest_password']) || $this->pluginObject->isSecretExpired() ) {
    ...
    private function setGuestLoginState() {
        ...
        switch (true) {
            case $this->isUserLoggedIn():
            case !$this->isPwEnabled():
                ilSession::set('guestLoggedIn', true);
                break;
            case trim($this->pluginObject->getAccessToken()) === trim($phrase);
    ...
    private function __construct()
    {
        ...
        ilMultiVcInit4Guestlink::initIlias();
