...
$ti = new ilTextInputGUI($pl->txt("zoom_account_id"), "svr_public_url");
$ti->setRequired(true);
$ti->setMaxLength(256);
$ti->setSize(60);
$ti->setInfo($pl->txt("zoom_account_info"));
$combo->addSubItem($ti);


$ti = new ilTextInputGUI($pl->txt("zoom_client_id"), "svr_username");
$ti->setRequired(true);
$ti->setMaxLength(256);
$ti->setSize(60);
//$ti->setInfo($pl->txt("zoom_client_id_info"));
$combo->addSubItem($ti);

// Password unreadable
$pi = new ilPasswordInputGUI($pl->txt("zoom_client_secret"), "svr_salt");
$pi->setSkipSyntaxCheck(true);
$pi->setRequired(true);
$pi->setMaxLength(256);
$pi->setSize(6);
//$pi->setInfo($pl->txt("zoom_client_secret_info"));
$pi->setRetype(false);
$combo->addSubItem($pi);

$cb = new ilCheckboxInputGUI($pl->txt("moderated_choose"), "cb_moderated_choose");
$cb->setRequired(false);
$cb->setInfo($pl->txt("moderated_choose_info") . " " . $pl->txt("config_help_begin") . " " . $pl->txt("moderated_zoom_info"));
$combo->addSubItem($cb);

$cb = new ilCheckboxInputGUI($pl->txt("moderated_default"), "cb_moderated_default");
$cb->setRequired(false);
$cb->setInfo($pl->txt("moderated_default_zoom_info"));
$combo->addSubItem($cb);

$cb = new ilCheckboxInputGUI($pl->txt("zoom_lobby_choose"), "extra_cmd_choose");
$cb->setRequired(false);
$cb->setInfo($pl->txt("zoom_lobby_choose_info"));
$combo->addSubItem($cb);

$si = new ilSelectInputGUI($pl->txt("zoom_lobby_default"), "extra_cmd_default");
$si->setOptions([
    0 => $pl->txt("zoom_lobby_0"),
    1 => $pl->txt("zoom_lobby_1"),
    2 => $pl->txt("zoom_lobby_2"),
    3 => $pl->txt("zoom_lobby_3"),
    4 => $pl->txt("zoom_lobby_4")
]);
$combo->addSubItem($si);
